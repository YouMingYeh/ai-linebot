...
import { HTTPFetchError, middleware } from "@line/bot-sdk";
...
app.post("/callback", middleware(middlewareConfig), async (req, res) => {
    ...
    const results = await Promise.all(events.map(async (event) => {
        try {
            ...
            return { status: "error", eventId: event.webhookEventId, error: (err instanceof Error) ? err.message : String(err) };
        }
        // Return success status for this specific event
        return { status: "success", eventId: event.webhookEventId };
    }));
