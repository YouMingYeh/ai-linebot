...
async function setupEnv() {
  ...
  console.log(
    chalk.yellow(`Before you begin, make sure you have completed these steps:`)
  );
  console.log(
    ` 1) Create a LINE Official Account: ${chalk.underline("https://entry.line.biz/form/entry/unverified")}`
  );
  ...
  console.log(
    `  LINE Developers Console: ${chalk.underline("https://developers.line.biz/console/")}` 
  );
  console.log(
    `  LINE Messaging API Docs: ${chalk.underline("https://developers.line.biz/en/docs/messaging-api/")}`
  );
  console.log(
    `  Google AI Studio:        ${chalk.underline("https://aistudio.google.com/apikey")}`
  );
  console.log(
    `  OpenAI API Keys:         ${chalk.underline("https://platform.openai.com/api-keys")}`
  );
  console.log(
    chalk.cyanBright("--------------------------------------------------\n")
  );
  ...
  try {
    ...
    console.warn(
      chalk.yellow("⚠️  No .env.example found—starting with empty defaults.")
    );
  }
  ...
  console.log(
    `   Go to the LINE Developers Console (${chalk.underline("https://developers.line.biz/console/")})`
  );
  console.log(
    `   Select your Provider → Select the Messaging API channel you just configured.`
  );
  ...
  answers.CHANNEL_ACCESS_TOKEN = await input({
    message:
      chalk.green(
        `👉 Paste your Channel Access Token (long-lived) [${chalk.italic(defaultValues["CHANNEL_ACCESS_TOKEN"] || "")}]:`
      ) +
      chalk.dim(
  ...
  answers.CHANNEL_SECRET = await input({
    message:
      chalk.green(
        `👉 Paste your Channel Secret [${chalk.italic(defaultValues["CHANNEL_SECRET"] || "")}]:`
      ) +
      chalk.dim(
  ...
  answers.DEFAULT_AI_PROVIDER = await select({
    message:
      chalk.green(
        `🤖 Default AI Provider [${chalk.italic(defaultValues["DEFAULT_AI_PROVIDER"] || "google")}]:`
      ) +
      chalk.dim(
  ...
  if (answers.DEFAULT_AI_PROVIDER === "google") {
    answers.GOOGLE_AI_MODEL = await input({
      message:
        chalk.green(
          `🔍 Google AI Model [${chalk.italic(defaultValues["GOOGLE_AI_MODEL"] || "gemini-2.0-flash-001")}]:`
    ...
    console.log(
      chalk.blueBright(
        `   ➡️ Get your Google API Key here: ${chalk.underline("https://aistudio.google.com/apikey")}`
      )
    );
    answers.GOOGLE_GENERATIVE_AI_API_KEY = await input({
      message:
        chalk.green(
          `🔑 Google API Key [${chalk.italic(defaultValues["GOOGLE_GENERATIVE_AI_API_KEY"] || "")}]:`
        ) +
        chalk.dim(
  ...
  if (answers.DEFAULT_AI_PROVIDER === "openai") {
    answers.OPENAI_MODEL = await input({
      message:
        chalk.green(
          `🔍 OpenAI Model [${chalk.italic(defaultValues["OPENAI_MODEL"] || "gpt-4o")}]:`
    ...
    console.log(
      chalk.blueBright(
        `   ➡️ Get your OpenAI API Key here: ${chalk.underline("https://platform.openai.com/api-keys")}`
      )
    );
    answers.OPENAI_API_KEY = await input({
      message:
        chalk.green(
          `🔑 OpenAI API Key [${chalk.italic(defaultValues["OPENAI_API_KEY"] || "")}]:`
        ) +
        chalk.dim(
